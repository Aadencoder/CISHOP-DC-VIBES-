...
function _send_auto_notify($update_id)
{
    ...
    //build a msg for customer
    $msg = "Order ".$order_ref.' has just been updated.';
    $msg.= "Current status: ".$status_title;
...
function view()
{
    ...
    if ($order_status == 0){
        $data['status_title'] = 'Order Submited';
    } else {
        $data['status_title'] = $this->cart->store_order_status->_get_status_title($order_status);
    }

    //fetch the content of the shopping cart
    $table = 'store_basket';
    $data['query'] = $this->cart->_fetch_cart_contents($session_id, $data['shopper_id'], $table);
...
function _generate_mysql_query($use_limit)
{
    ...
    if ($order_status > 0) {
        $mysql_query = "SELECT store_orders.id,
                     ...
                     FROM store_orders INNER JOIN store_accounts ON store_orders.shopper_id = store_accounts.id INNER JOIN store_order_status ON store_orders.order_status = store_order_status.id WHERE store_orders.order_status = $order_status ORDER BY store_orders.date_created DESC ";
    } else {
           $mysql_query = "SELECT store_orders.id,
                               store_orders.order_ref,
                               store_orders.date_created,
                               ...
                               store_accounts.lastname,
                               store_accounts.company
                       FROM store_orders INNER JOIN store_accounts ON store_orders.shopper_id = store_accounts.id WHERE store_orders.order_status = $order_status ORDER BY store_orders.date_created DESC";
...
{
    //find the totatl amount taken from this order
...
{
    //this gets called  from the paypal IPN listener, when anorder is placed
    $this->load->module('site_security');
    ...
    //transfer from store basket to shopper track..\
