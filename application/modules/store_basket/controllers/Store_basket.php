...
function _avoid_cart_conflicts($data)
{
    ...
    if ($num_rows > 0)//conflicting in shopertrack 
     {
        //regenerate the session id 
        session_regenerate_id();

        //& update data session id variable
...
function add_to_basket()
{
    ...
    if ($submit == "Submit"){
        ...
        $this->form_validation->set_rules('item_size' , 'Item Price', 'numeric');
...
function _fetch_the_data()
{
    ...
    //get all the data from the store_ basket table
...
{
    //NOTE : VALUE type can be color OR size
    if ($value_type == 'size'){
