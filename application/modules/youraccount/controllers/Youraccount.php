...
function test1()
{	
	$your_name = "metalhead101";
	$this->session->set_userdata('your_name', $your_name);
	echo "session variable was set";
	echo "<hr>";

	echo anchor('youraccount/test2', 'Display the session variable.<br>');
	echo anchor('youraccount/test3', 'Destroy the session variable.<br>');
...
function test2()
{
	...
	if ($your_name!= ''){
		echo "<h1>Helllo $your_name</h1>";
	} else {
		echo "No sessin variable is set.";
	}
	echo "<hr>";
...
{
	//NOTE::login type can be long term or shortterm
	...
	//attemp to update and divert the cart
...
function _attempt_cart_divert($user_id)
{
	...
	if ( $num_rows > 0 ) {
		// there are records to be updated
