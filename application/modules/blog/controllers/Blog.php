...
function _generate_multiple_thumbnail($file_name, $thumbnail_name)
{
    $config['image_library'] = 'gd2';
    $config['source_image'] = './blog_pics/'.$file_name;
    $config['new_image'] = './blog_pics/'.$thumbnail_name;
...
function test()
{
    $length = '32';
    $this->load->module('site_security');
    $str = $this->site_security->generate_random_number($length);
    echo $str;
}
...
function create()
{
    ...
    if ($submit == "Submit"){
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (is_numeric($update_id)) {
                $this->_update($update_id, $data);
                $flash_msg = "The blog was successfully updated.";
                $value = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item', $value);
                redirect('blog/create/'.$update_id);
            } else {
    ...
    if ($data['date_published'] > 0) {
        //it is unix time convert to nice format
