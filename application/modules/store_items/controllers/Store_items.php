...
function view($update_id)
{
    if(!is_numeric($update_id)) {
     ...
     $data['item_price_desc'] = str_replace('.00', 'replace', $data['item_price_desc']);
...
function _generate_breadcrumbs_array($update_id)
{
    ...
    //figure out the sub_cat_id
    $sub_cat_id = $this->_get_sub_cat_id($update_id);

    //get the sub cat_title
...
{
    //figure out which sub cat has more item 
    // what the sub cat id is 
    ...
    if ($num_sub_cats_for_item == 1) {
        //the item has only 1 subcat
        $sub_cat_id = $potential_sub_cats['0'];
        return $sub_cat_id;
    } else {
        //we have more than one sub cat
...
function _process_delete($update_id)
{
    ...
    $small_pic_path = './big_pics/'.$small_pic;
...
function delete_image($update_id)
{
    ...
    $flash_msg = "The item image was successfully removed.";
    $value = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
    $this->session->set_flashdata('item', $value);
...
function create()
{
    ...
    if ($submit == "Submit"){
        ...
        $this->form_validation->set_rules('was_price' , 'Wass Price', 'required|numeric');
        $this->form_validation->set_rules('status' , 'Item Status', 'required|numeric');
...
{   
    ...
    if ($num_rows>0) {
        $this->form_validation->set_message('item_check', 'The title you have submitted is not available');
        return FALSE;
    } else {
        return TRUE;
    }
