    ...
    if ($submit == "Submit"){
        ...
        if ($this->form_validation->run() == TRUE) {
           ...
           $data['pword'] = $this->site_security->_hash_string($pword);
                ...
                $flash_msg = "The acccount password were successfully updated.";
...
function create()
{
    ...
    if ($submit == "Submit"){
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (is_numeric($update_id)) {
                $this->_update($update_id, $data);
                $flash_msg = "The acccount details were successfully updated.";
                $value = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item', $value);
                redirect('store_accounts/create/'.$update_id);
            } else {
...
function delete($update_id)
{
   if(!is_numeric($update_id)) {
    ...
    if ( $submit == "Cancel") {
        ...
        if ($allowed == FALSE) {
            $flash_msg = "The are not allowed to delete data.";
