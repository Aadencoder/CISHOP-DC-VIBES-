...
{
    $mysql_query = "SELECT btm_nav.*, webpages.* FROM btm_nav INNER JOIN webpages ON btm_nav.page_id = webpages.id ORDER BY btm_nav.priority";
...
function _draw_shortable_list(){
    $mysql_query = "SELECT btm_nav.*, webpages.* FROM btm_nav INNER JOIN webpages ON btm_nav.page_id = webpages.id ORDER BY btm_nav.priority";
    $data['query'] = $this->_custom_query($mysql_query);
    $data['special_pages'] = $this->get_special_page();
...
function view($update_id)
{
    if(!is_numeric($update_id)) {
     ...
     //count the item that belong to this homepage offer
     $use_limit = FALSE;
...
{   
    //NOTE :  user limit can be true/ffalse
     $mysql_query = "SELECT * FROM store_cat_assign INNER JOIN store_items ON store_cat_assign.item_id = store_items.id WHERE store_cat_assign.cat_id = '$update_id' AND store_items.status=1";
    ...
    if ($submit == "Submit"){
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (is_numeric($update_id)) {
                ...
                //insert net homepage offer
                $this->_insert($data);
                $update_id = $this->get_max(); //get the id of new homepage offer
