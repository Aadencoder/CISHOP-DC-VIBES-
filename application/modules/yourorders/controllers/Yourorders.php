...
{
    //return all order status options
...
function view()
{
    ...
    //query binding test
    // $mysql_query ="SELECT store_order_status.status_title, store_orders.* FROM store_orders INNER JOIN store_order_status ON store_orders.order_status = store_order_status.id WHERE store_orders.order_ref = ?";
    ...
    $data['date_created'] = $this->timedate->get_nice_date($date_created, 'mini');
    if ($order_status==0){
