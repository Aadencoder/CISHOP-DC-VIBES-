...
function _is_valid_item($update_id)
{
...
function submit($update_id)
{
 if(!is_numeric($update_id)) {
    ...
    if ($submit == "Finished") {
        ...
        $is_valid_item = $this->_is_valid_item($update_id);
        if($is_valid_item == FALSE){
             $flash_msg = "The item id is invalid.";
        ...
        if ($item_id!==""){
            ...
            $flash_msg = "The new item_id option was successfully added.";
            $value = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
            $this->session->set_flashdata('item', $value);
        }
    }
...
function update($update_id)
{
    ...
    $data['headline'] = "Upload Homepage Offer";
